	...
	public static void main(String[] args) {
        ...
        try {
        	...
        	while (true) {
	            ...
	            int[] results = processor.analyzeSentence();
	            byte[] outData = processor.convertToByteArray(totalCharacters);
	            byte[] vowels = processor.convertToByteArray(results[0]);
	            byte[] consonants = processor.convertToByteArray(results[1]);
	            byte[] punctuations = processor.convertToByteArray(results[2]);
	            
	            // 7. Get the client information
	            InetAddress clientAddress =  receivedPacket.getAddress();
	            int clientPort = receivedPacket.getPort();
	            int sizeOutData = outData.length;
	            int vowelDataSize = vowels.length;
	            int consonantsDataSize = consonants.length;
	            int punctuationsDataSize = punctuations.length;
	            
	            // 8. Wrap data into datagram packet
	            DatagramPacket outPacket = new DatagramPacket(outData, 
	            		sizeOutData, clientAddress, clientPort);
	            DatagramPacket vowelPacket = new DatagramPacket(vowels, 
	            		vowelDataSize, clientAddress, clientPort);
	            DatagramPacket consonantPacket = new DatagramPacket(consonants, 
	            		consonantsDataSize, clientAddress, clientPort);
	            DatagramPacket punctuationPacket = new DatagramPacket(punctuations, 
	            		punctuationsDataSize, clientAddress, clientPort);
	            ...
	            System.out.println("Message sent (totalCharacters) : " 
	            		+ totalCharacters  + ".");
	            System.out.println("Message sent (vowels) : " 
	            		+ results[0]  + ".");
	            System.out.println("Message sent (consonants) : " 
	            		+ results[1]  + ".");
	            System.out.println("Message sent (punctuations) : " 
	            		+ results[2]  + ".\n");
